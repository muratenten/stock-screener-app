        ...
        if portfolio_str:
            try:
                ...
                if isinstance(portfolio_dict, dict) and ("purchase_records" in portfolio_dict or "watchlist" in portfolio_dict or "sales_records" in portfolio_dict):
                    ...
                    # Timestamp safety check: do not overwrite newer session or local file data
                    current_portfolio = st.session_state.get("portfolio_cache")
                    curr_ts = current_portfolio.get("last_updated", "") if isinstance(current_portfolio, dict) else ""
                    incoming_ts = portfolio_dict.get("last_updated", "")
                    if curr_ts and incoming_ts and curr_ts > incoming_ts:
...
def render_yutai_section(ticker, name, metrics):
    ...
    genre = yutai_data.get('genre', '自社商品・優待券')
        ...
        with sim_container2:
            ...
            if owned_rec and owned_rec["quantity"] > 0:
                ...
                if st.button(sell_btn_label, type="secondary", use_container_width=True, key=f"sim_sell_btn_{selected_ticker}{key_suffix}"):
                    execute_virtual_sell(selected_ticker, sell_target_qty, current_price_val)
...
def load_portfolio(user_key=None):
    ...
    # If in session cache for this user, use it
    if "portfolio_cache" in st.session_state and isinstance(st.session_state["portfolio_cache"], dict):
        ...
        if "purchase_records" in cache_data or "sales_records" in cache_data or "watchlist" in cache_data:
            if st.session_state.get("user_tier") == "premium":
...
def execute_virtual_sell(ticker, qty_to_sell, curr_price=None):
    ...
    # Matching records for ticker
    matching = [r for r in p_records if r.get("ticker") == ticker]
    if not matching:
        st.error(f"銘柄コード {ticker} の保有レコードが見つかりませんでした。")
        return False
        
    target_rec = matching[0]
    total_qty_owned = sum(float(r.get("quantity", 0)) for r in matching)
    
    if curr_price is None or pd.isna(curr_price) or curr_price <= 0:
        curr_price = float(target_rec.get("purchase_price", 0.0))
    else:
        curr_price = float(curr_price)
        
    qty_to_sell = float(qty_to_sell)
    actual_sell_qty = min(qty_to_sell, total_qty_owned)
    if actual_sell_qty <= 0:
        st.error("売却株数は1株以上である必要があります。")
        return False
        
    p_price = float(target_rec.get("purchase_price", curr_price))
    rate = get_usdjpy_rate() if is_us_stock(ticker) else 1.0
    
    realized_pl_val = (curr_price - p_price) * actual_sell_qty
    realized_pl_jpy_val = realized_pl_val * rate
    
    # Record sale
    s_records.append({
        "ticker": ticker,
        "name": target_rec.get("name", ticker),
    ...
    # Cumulative Realized PL
    portfolio["total_realized_pl_jpy"] = float(portfolio.get("total_realized_pl_jpy", 0.0)) + float(realized_pl_jpy_val)
    
    # If 100% (or within floating precision) is sold, completely eliminate all purchase records for this ticker
    is_100_percent = (actual_sell_qty >= total_qty_owned - 1e-4)
    if is_100_percent:
        portfolio["purchase_records"] = [r for r in p_records if r.get("ticker") != ticker]
    else:
        # Partial sale: update remaining quantity and invest amount
        new_qty = total_qty_owned - actual_sell_qty
        target_rec["quantity"] = float(new_qty)
        target_rec["invest_amount"] = max(0.0, float(new_qty * p_price))
        portfolio["purchase_records"] = [r for r in p_records if r.get("ticker") != ticker] + [target_rec]
        
    # Save portfolio
    saved = save_portfolio(portfolio)
    if saved:
        # Clean up widget states related to this ticker
        if "sell_active_ticker" in st.session_state:
            del st.session_state["sell_active_ticker"]
        for k in list(st.session_state.keys()):
            if "sell_qty" in k and ticker in k:
                del st.session_state[k]
                
        # Set flash message & sell dialog popup
        stock_name = target_rec.get("name", ticker)
        qty_display = int(actual_sell_qty) if int(actual_sell_qty) == actual_sell_qty else actual_sell_qty
        st.session_state['flash_sell_msg'] = f"🎉 **{stock_name} ({ticker})** を {qty_display:,}株 正常に売却しました！（確定損益: {int(realized_pl_jpy_val):+,}円）"
...
if 'firebase_loaded_users' not in st.session_state:
    st.session_state['firebase_loaded_users'] = set()

if user_key and user_key != 'default' and user_key not in st.session_state['firebase_loaded_users']:
    st.session_state['firebase_loaded_users'].add(user_key)
    firebase_project_id = st.session_state.get('firebase_project_id', DEFAULT_FIREBASE_PROJECT_ID)
    if firebase_project_id:
        id_token = st.session_state.get("firebase_id_token")
        ...
        if is_ready:
            ...
            if val_str:
                try:
                    ...
                    if isinstance(browser_portfolio_data, dict) and ("purchase_records" in browser_portfolio_data or "watchlist" in browser_portfolio_data or "sales_records" in browser_portfolio_data):
                        # Check timestamp to avoid overwriting newer local edits
                        local_ts = local_portfolio_data.get("last_updated", "") if isinstance(local_portfolio_data, dict) else ""
                        browser_ts = browser_portfolio_data.get("last_updated", "")
                        if not (local_ts and browser_ts and local_ts > browser_ts):
                            filename = get_portfolio_filename()
                            with open(filename, "w", encoding="utf-8") as f:
                                json.dump(browser_portfolio_data, f, indent=4, ensure_ascii=False)
                            st.session_state["portfolio_cache"] = browser_portfolio_data
        ...
        if 'screening_results' in st.session_state:
            ...
            if not results:
                ...
                with st.expander("📖 良い銘柄を見極めるための詳細ガイド（各指標の役割と判断基準）", expanded=False):
                    st.markdown("""
                    <div style="overflow-x: auto; font-size: 0.88rem; line-height: 1.6;">
                        <p style="margin-top:0; color: var(--text-color, #1e293b);">株式投資で「優良銘柄（お宝株）」を探す際の目安となる数値基準一覧です。スクリーニング結果の比較にご活用ください。</p>
                        <table style="width: 100%; border-collapse: collapse; text-align: left; margin-bottom: 12px;">
                            <thead>
                                <tr style="border-bottom: 2px solid rgba(100, 116, 139, 0.2); color: var(--text-color, #1e293b);">
                                    <th style="padding: 8px 6px;">指標名</th>
                                    <th style="padding: 8px 6px;">意味（何を見る指標か）</th>
                                    <th style="padding: 8px 6px;">🟢 良い銘柄の目安数値</th>
                                    <th style="padding: 8px 6px;">見方と投資判断のポイント</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr style="border-bottom: 1px solid rgba(100, 116, 139, 0.1);">
                                    <td style="padding: 8px 6px; font-weight: bold;">PER (株価収益率)</td>
                                    <td style="padding: 8px 6px;">稼ぐ力（利益）に対する割安度</td>
                                    <td style="padding: 8px 6px; color: #16a34a; font-weight: bold;">15倍 未満<br><span style="font-size:0.75rem; color:#64748b;">(10倍以下は超割安)</span></td>
                                    <td style="padding: 8px 6px; color: #64748b;">投資元本を会社の純利益で回収できる年数。15倍以下はお買い得。成長株は30倍以上になることも。</td>
                                </tr>
                                <tr style="border-bottom: 1px solid rgba(100, 116, 139, 0.1);">
                                    <td style="padding: 8px 6px; font-weight: bold;">PBR (株価純資産倍率)</td>
                                    <td style="padding: 8px 6px;">会社の財産（純資産）に対する割安度</td>
                                    <td style="padding: 8px 6px; color: #16a34a; font-weight: bold;">1.0倍 未満<br><span style="font-size:0.75rem; color:#64748b;">(0.8倍以下は超割安)</span></td>
                                    <td style="padding: 8px 6px; color: #64748b;">会社が解散した時の財産価値。1.0倍未満は「会社の純資産より株価が安い」状態（東証も改善要請中）。</td>
                                </tr>
                                <tr style="border-bottom: 1px solid rgba(100, 116, 139, 0.1);">
                                    <td style="padding: 8px 6px; font-weight: bold;">ROE (自己資本利益率)</td>
                                    <td style="padding: 8px 6px;">預かったお金を増やす上手さ</td>
                                    <td style="padding: 8px 6px; color: #2563eb; font-weight: bold;">8% 〜 10% 以上</td>
                                    <td style="padding: 8px 6px; color: #64748b;">株主資本を使ってどれだけ効率よく利益を生み出しているか（資本効率の高さ）。10%以上は優良企業。</td>
                                </tr>
                                <tr style="border-bottom: 1px solid rgba(100, 116, 139, 0.1);">
                                    <td style="padding: 8px 6px; font-weight: bold;">配当利回り</td>
                                    <td style="padding: 8px 6px;">年間の現金配当リターン</td>
                                    <td style="padding: 8px 6px; color: #f59e0b; font-weight: bold;">3.0% 〜 4.0% 以上</td>
                                    <td style="padding: 8px 6px; color: #64748b;">株価に対する年間配当の割合。3%以上は高配当株と呼ばれ、下落相場でも株価の下値が堅い。</td>
                                </tr>
                                <tr style="border-bottom: 1px solid rgba(100, 116, 139, 0.1);">
                                    <td style="padding: 8px 6px; font-weight: bold;">売上高成長率</td>
                                    <td style="padding: 8px 6px;">本業のビジネス規模の拡大スピード</td>
                                    <td style="padding: 8px 6px; color: #2563eb; font-weight: bold;">前年比 +10% 以上</td>
                                    <td style="padding: 8px 6px; color: #64748b;">会社の売上が前年と比べて拡大しているスピード。2桁成長（+10%以上）は事業拡大の証。</td>
                                </tr>
                                <tr style="border-bottom: 1px solid rgba(100, 116, 139, 0.1);">
                                    <td style="padding: 8px 6px; font-weight: bold;">EPS成長率</td>
                                    <td style="padding: 8px 6px;">1株あたり純利益の伸び率</td>
                                    <td style="padding: 8px 6px; color: #2563eb; font-weight: bold;">前年比 +15% 以上</td>
                                    <td style="padding: 8px 6px; color: #64748b;">1株あたりの最終利益の成長率。株価の上昇トレンドに最も直結しやすい重要指標。</td>
                                </tr>
                                <tr>
                                    <td style="padding: 8px 6px; font-weight: bold;">営業利益率</td>
                                    <td style="padding: 8px 6px;">本業の儲かりやすさ（利益の質）</td>
                                    <td style="padding: 8px 6px; color: #16a34a; font-weight: bold;">8% 〜 10% 以上</td>
                                    <td style="padding: 8px 6px; color: #64748b;">売上高に対してどれだけ本業で利益が残るか。10%超えは業界内での高い競争力を示す。</td>
                                </tr>
                            </tbody>
                        </table>
                        <div style="margin-top: 10px; font-size: 0.85rem; color: #64748b;">
                            <strong>🎯 スクリーニングでの組み合わせのコツ:</strong><br>
                            ・<strong>堅実な高配当・割安バリュー投資</strong>: <code>PER < 15倍</code> ＋ <code>PBR < 1.0倍</code> ＋ <code>配当利回り >= 3%</code> ＋ <code>ROE >= 8%</code><br>
                            ・<strong>株価上昇を狙う成長株投資</strong>: <code>売上高成長率 >= 10%</code> ＋ <code>EPS成長率 >= 15%</code> ＋ <code>ROE >= 10%</code> ＋ <code>上昇トレンド</code>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                
                # Render helpful criteria summary banner
                st.markdown("""
                <div style="background: rgba(59, 130, 246, 0.05); border: 1px solid rgba(59, 130, 246, 0.18); border-radius: 8px; padding: 10px 14px; margin: 10px 0 12px 0; font-size: 0.84rem; display: flex; flex-wrap: wrap; gap: 8px 16px; align-items: center; line-height: 1.5; color: var(--text-color, #334155);">
                    <div style="font-weight: 700; color: #2563eb; display: flex; align-items: center; gap: 4px;">
                        <span>💡</span><span>優良の目安:</span>
                    </div>
                    <span><strong>PER</strong>: 15倍以下 (割安)</span>
                    <span><strong>PBR</strong>: 1.0倍以下 (割安)</span>
                    <span><strong>ROE</strong>: 8%以上</span>
                    <span><strong>配当</strong>: 3.0%以上</span>
                    <span><strong>売上成長</strong>: +10%以上</span>
