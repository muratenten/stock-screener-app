...
def extract_single_fundamental(ticker, ticker_meta=None):
    try:
        ...
        return {
            ...
            'bps': bps,
            'dps': dps,
            'dividend_yield': div_y if 'div_y' in locals() and div_y is not None else None,
            'per': per if 'per' in locals() and per is not None else None,
            'pbr': pbr if 'pbr' in locals() and pbr is not None else None,
            'roe': roe,
            'op_margin': op_margin,
